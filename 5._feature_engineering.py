...
def main(args: argparse.Namespace) -> tuple[np.ndarray, np.ndarray]:
    ...
    for col in range(train_data_T.shape[0]):
        if all([i.is_integer() for i in train_data_T[col]]):
            int_cols.append(col)
        else:
            other_cols.append(col)

    col_trans = sklearn.compose.ColumnTransformer( [
        ('OneHotEncoder', sklearn.preprocessing.OneHotEncoder(sparse=False, handle_unknown="ignore"), int_cols),
        ("StandardScaler", sklearn.preprocessing.StandardScaler(), other_cols)
    ] )
