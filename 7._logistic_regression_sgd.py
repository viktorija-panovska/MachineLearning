...
def accuracy(data, target, weights):
    predictions = [int(predict_at(x, weights) > 0.5) for x in data]
    return sum([y_true == y_pred for y_true, y_pred in zip(target, predictions)]) / len(target)

def loss(data, target, weights):
    return -sum([ target[i] * np.log(predict_at(data[i], weights)) + (1 - target[i]) * np.log(1 - predict_at(data[i], weights)) for i in range(len(target))]) / len(target)
...
def main(args: argparse.Namespace) -> tuple[np.ndarray, list[tuple[float, float]]]:
    ...
    for epoch in range(args.epochs):
        ...
        for batch in permutation:
            gradient = 0
            for i in batch:
                gradient += (1/args.batch_size) * np.dot(sigmoid(np.dot(train_data[i].T, weights)) - train_target[i], train_data[i])
        ...
        test_loss = loss(test_data, test_target, weights)
